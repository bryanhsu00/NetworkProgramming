...
void NumberPipe::erase_in_pipe(){
    if(inflag != -1){
        ...
        pidlist.erase(pidlist.begin() + inflag);
    }
}
...
void NumberPipe::add_pid(pid_t pid){
    if(outflag != -1){
        pidlist[outflag].push_back(pid);
    }
    if(errflag != -1){
        pidlist[errflag].push_back(pid);
    }
}

vector<pid_t> NumberPipe::get_pid(){
    return pidlist[inflag];
}